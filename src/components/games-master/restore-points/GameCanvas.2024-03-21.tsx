...
export default function GameCanvas({ selectedCharacter, ringConfig, completionEffect, isGameStarted }: GameCanvasProps) {
  ...
  const fireworksRef = useRef<(PIXI.Graphics | Function)[]>([])
  ...
  const createFirework = (x: number, y: number) => {
    ...
    for (let i = 0; i < 20; i++) { // Reduced from 50 to 20 particles per firework
  ...
  const showFireworks = () => {
    ...
    for (let i = 0; i < 3; i++) { // Reduced from 5 to 3 fireworks
      setTimeout(() => {
